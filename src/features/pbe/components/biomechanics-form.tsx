...
export function BiomechanicsForm({ initialData, patientId, patientName, patientEmail, patientPhone, onSave, onChange, readOnly }: BiomechanicsFormProps) {
    const DEFAULT_DATA = {
        ...
        const mergeDefaults = (input: any, defaults: any) => {
            ...
            return {
                ...
                efep: {
                    ...defaults.efep,
                    ...(input.efep || {}),
                    items: (input.efep?.items && input.efep.items.length > 0) ? input.efep.items : defaults.efep.items
                },
    ...
    const updateField = (path: string, val: any) => {
        ...
        setData((prev: any) => {
            ...
            for (let i = 0; i < keys.length - 1; i++) {
                ...
                // [FIX] Correctly clone arrays to avoid converting them to objects
    ...
    // Sync with parent immediately on change (for Finalize/Report actions)
    useEffect(() => {
