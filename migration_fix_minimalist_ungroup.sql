...
BEGIN
    ...
    -- Update: Replace the entire "Minimalist Index" section with "Ungrouped" version.
    -- Since we use JSONB concatenation usually, we might have duplicates if we just append.
    -- Ideally we would replace, but since the user has "two templates" and confusion, 
    -- let's just Append the CORRECTED version at the end and clear up the old one manually in UI or via script if I dare (risky to delete by index).
    -- User asked: "Separate each item... into a single field".
    
    new_fields := '[
        {"type": "section", "label": "Índice de Minimalismo (Corrigido)", "textAlign": "center", "fontSize": "xl"},
